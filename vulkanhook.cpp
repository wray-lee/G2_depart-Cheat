...
namespace hooks_vk {
    ...
    static bool SafeGetDeviceQueue(PFN_vkGetDeviceQueue pGetQueueFunc, VkDevice device, uint32_t familyIndex, uint32_t queueIndex, VkQueue* pQueue)
    {
        __try
        {
            pGetQueueFunc(device, familyIndex, queueIndex, pQueue);
            return true;
        }
    ...
    static bool IsPlausibleDevice(VkDevice dev)
    {
        ...
        if (oGetDeviceProcAddr)
        {
            ...
            if (SafeGetDeviceQueue(p, dev, 0, 0, &q))
            {
